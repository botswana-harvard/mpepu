...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        DispensingIdentifierModel = orm['ph_dispenser.DispensingIdentifierModel']
        IdentifierSequence = orm['ph_dispenser.IdentifierSequence']

        obj = DispensingIdentifierModel.objects.aggregate(Max('sequence_number'))
        max2 = obj.get('sequence_number__max', 0)
        if not max2:
            max2 = 0
        obj = IdentifierSequence.objects.aggregate(Max('id'))
        max1 = obj.get('id', 0)
        if not max1:
            max1 = 0
        for n in range(max1, max2 + 1):
