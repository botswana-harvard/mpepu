...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        MaternalRequisition = orm['mpepu_lab.MaternalRequisition']
        InfantRequisition = orm['mpepu_lab.InfantRequisition']

        OldPanel = orm['lab_panel.Panel']
        NewPanel = orm['lab_clinic_api.Panel']
        OldAliquotType = orm['lab_aliquot_list.AliquotType']
        NewAliquotType = orm['lab_clinic_api.AliquotType']
        for pk in [x.get('aliquot_type_id') for x in InfantRequisition.objects.values('aliquot_type_id').all().distinct()]:
            old_aliquot_type = OldAliquotType.objects.get(pk=pk)
            new_aliquot_type, created = NewAliquotType.objects.get_or_create(name=old_aliquot_type.name)
            InfantRequisition.objects.filter(old_aliquot_type_id=old_aliquot_type.pk).update(aliquot_type=new_aliquot_type)
        for pk in [x.get('aliquot_type_id') for x in MaternalRequisition.objects.values('aliquot_type_id').all().distinct()]:
            old_aliquot_type = OldAliquotType.objects.get(pk=pk)
            new_aliquot_type, created = NewAliquotType.objects.get_or_create(name=old_aliquot_type.name)
            MaternalRequisition.objects.filter(old_aliquot_type_id=old_aliquot_type.pk).update(aliquot_type=new_aliquot_type)
        # update requisitions with a new panel from lab_clinic_api
        for pk in [x.get('panel_id') for x in InfantRequisition.objects.values('panel_id').all().distinct()]:
            old_panel = OldPanel.objects.get(pk=pk)
            new_panel, created = NewPanel.objects.get_or_create(name=old_panel.name)
            InfantRequisition.objects.filter(old_panel_id=old_panel.pk).update(panel=new_panel)
        for pk in [x.get('panel_id') for x in InfantRequisition.objects.values('panel_id').all().distinct()]:
            old_panel = OldPanel.objects.get(pk=pk)
            new_panel, created = NewPanel.objects.get_or_create(name=old_panel.name)
            MaternalRequisition.objects.filter(old_panel_id=old_panel.pk).update(panel=new_panel)
