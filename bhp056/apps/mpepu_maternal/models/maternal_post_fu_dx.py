...
class MaternalPostFuDx(BaseScheduledVisitModel):
    ...
    mother_hospitalized = models.CharField(
        max_length=3,
        choices=YES_NO,
        verbose_name="Has the mother been hospitalized overnight since the last scheduled visit (or since discharge after delivery, if this is the randomization visit)?",
    ...
    new_diagnoses = models.CharField(
        max_length=3,
        choices=YES_NO,
        verbose_name="Since the last attended scheduled visit, has the mother had any of the following which were NEW (never previously reported, or a NEW episode of a previously resolved* diagnosis)",
