...
class MaternalEnrollArv(BaseScheduledVisitModel):
    ...
    haart_start_date = models.DateField(
        verbose_name="1b. Date of HAART first started",
        help_text="",
        )
    ...
    preg_on_haart = models.CharField(
        max_length=25,
        choices=YES_NO,
        verbose_name="1c. Was she still on HAART at the time she became pregnant for this pregnancy? ",
        help_text="",
        )
    haart_changes = models.IntegerField(
        verbose_name="1d. How many times did you change your HAART medicines?",
        help_text="",
        )
    prior_preg = models.CharField(
        max_length=80,
        verbose_name="1e. Prior to this pregnancy the mother has ",
        choices=PRIOR_PREG_HAART_STATUS,
        help_text="",
        )
    prior_arv = models.ManyToManyField(PriorArv,
        verbose_name="1f. Please list all of the ARVs that the mother ever received prior to the current pregnancy:",
        help_text="",
        )
    prior_arv_other = OtherCharField(
        max_length=35,
        verbose_name="1g. if other specify...",
        blank=True,
        null=True,
        )
