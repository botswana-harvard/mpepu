...
class ArvDiscontinuationForm(BaseConsentedModelForm):
    def clean(self):
        ...
        if cleaned_data.get('stopped_once')=='Yes' and not cleaned_data.get('last_ftc_date') or cleaned_data.get('last_tdf_date') or cleaned_data.get('last_tdf_date') or cleaned_data.get('last_3tc_date'):
