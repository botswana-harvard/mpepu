...
class InfantFuForm (BaseInfantModelForm):
    def clean(self):
        ...
        if cleaned_data['diarrhea_illness'].upper() == 'NO':
            if InfantFuD.objects.filter(infant_fu__infant_visit=cleaned_data['infant_visit']).exists():
        ...
        if cleaned_data['has_dx'].upper() == 'NO':
            if InfantFuDx.objects.filter(infant_fu__infant_visit=cleaned_data['infant_visit']):
...
class InfantFuPhysicalForm (BaseInfantModelForm):
    ...
    def clean(self):
        ...
        if cleaned_data['was_hospitalized'] == 'Yes' and not cleaned_data ['days_hospitalized']:
            raise forms.ValidationError('If patient was hospitalised, give number of days in hospital')
        if cleaned_data['was_hospitalized'] == 'No' and cleaned_data ['days_hospitalized']:
    ...
    def clean(self):
        ...
        if 'specify' in cleaned_data.get('fu_dx', None):
            if not cleaned_data.get('fu_dx_specify', None):
                raise forms.ValidationError('please specify \'%s\'' % (cleaned_data['fu_dx'].replace('specify', ''),))
       ...
       #validating for eae report
        if cleaned_data['is_eae_required'] == 'Yes' and not cleaned_data ['eae_reference']:
        ...
        infant_fu_dx = cleaned_data['infant_fu_dx']
        ...
        if InfantFuPhysical.objects.filter(infant_visit=visit).exists():
            ...
            if infant_fu_physical.was_hospitalized <> cleaned_data['was_hospitalized']:
...
class InfantFuDx2ProphItemsForm (BaseInfantModelForm):
    def clean(self):
        ...
        if self.cleaned_data['dx'] == 'Yes' and not self.cleaned_data ['ctx'] and not self.cleaned_data ['nvp']:
    ...
    def clean(self):
        ...
        if 'vaccination' in cleaned_data:
            self.validate_m2m(
                    ...
                    leading=cleaned_data['vaccines_received'],
                    m2m=cleaned_data['vaccination'])
        return cleaned_data
