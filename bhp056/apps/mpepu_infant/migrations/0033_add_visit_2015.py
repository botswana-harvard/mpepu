...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        #content_type_map = ContentTypeMap.objects.get(content_type__model='infantpreeligibility')
        #membership_form = MembershipForm.objects.create(content_type_map=content_type_map, category='infant')
        #schedule_group = ScheduleGroup.objects.create(membership_form=membership_form, group_name='Infant Pre-eligibility')

        content_type_map = ContentTypeMap.objects.get(content_type__model='infantpreeligibility')
        membership_form = MembershipForm.objects.get(content_type_map=content_type_map, category='infant')
        schedule_group = ScheduleGroup.objects.get(membership_form=membership_form, group_name='Infant Pre-eligibility')

        visit_definition_helper = VisitDefinitionHelper()
        VisitDefinition = orm['bhp_visit.VisitDefinition']
        if not VisitDefinition.objects.get(code='2010'):
            raise TypeError('Data migration requires infant visit 2010 to be in the DB, not found.')
        else:
            visit_definition = VisitDefinition.objects.get(code='2010')
            visit_definition.base_interval = 27
            visit_definition.base_interval_unit = 'D'
            visit_definition.save()
        if not VisitDefinition.objects.filter(code='2015'):
            source = VisitDefinition.objects.get(code='2010')
            visit_definition_helper.copy(source, code='2015', title='Randomization II', time_point=15, base_interval=27, base_interval_unit='D', grouping='infant')
            visit_definition.schedule_group.all().delete()
            visit_definition = VisitDefinition.objects.get(code='2015')
